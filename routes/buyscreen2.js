    ...
    if(sess.email){
		...
		MongoClient.connect(url, function(err, db) {
		    ...
		    if (req.query.a != undefined) {
		    	var val = req.query.a;
		    	var cursor = db.collection('ads').find({'category':val});		   
			    cursor.each(function(err, doc) {
			    	if(doc != null){
			    		json_ads.push(doc);	
			    	}
			    	else {
			    		res.render('buyscreen2', { title: 'buyscreen2', name122:sess.name, array:json_ads});
			    	}
			    });
		    }
		    	...
		    	var value1 = val[0];
		    	var value2 = val[1];
		    	var cursor = db.collection('ads').find({'category':value1});		   
			    cursor.each(function(err, doc) {
			    	if(doc != null){
			    		var string = (doc.name).toLowerCase();
